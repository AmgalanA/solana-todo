...
export function useTodo() {
  ...
  useEffect(() => {
    // Fetch user profile if there is a profile then het its todo_accounts
  ...
  const initializeUser = async () => {
    // Check if the program exists and wallet is connected
    // then we want to run initializeUser() from smart contract
    if (program && publicKey) {
      try {
        ...
        setInitialized(true);
        toast.success("SUCCESS INITIALIZED");
      } catch (error) {
        console.log(error);
    ...
    if (program && publicKey) {
      try {
        ...
        if (input) {
          ...
          toast.apply("Successfully added todo.");
        }
      } catch (error) {
        console.log(error);
  ...
  const markTodo = async (todoPda, todoIdx) => {
    if (program && publicKey) {
      try {
        ...
        toast.success("SUCESSFULLY MARKED TODO!");
      } catch (error) {
        console.log(error);
        toast.error(error.toString());
      } finally {
  ...
  const removeTodo = async (todoPda, todoIdx) => {
    if (publicKey) {
      try {
        ...
        const [profilePda, profileDump] = findProgramAddressSync(
          [utf8.encode("USER_STATE"), publicKey.toBuffer()],
          program.programId
        );
        ...
        toast.success("REMOVING TODO SUCCESS!");
      } catch (error) {
        console.log(error);
        toast.error(error);
      } finally {
